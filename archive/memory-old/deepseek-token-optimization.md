...
## 模型路由（2026-06-21 决策）
- **当前策略**：v4pro 默认 + Agent 分流（策略 B）
  - 主会话保持 v4pro，不改变默认模型
  - spawn agent 时简单任务用轻量模型：`agent("读文件...", {model: "haiku"})`
  - 复杂 agent（审查、重构）仍用 v4pro/opus
- **原因**：当前无 CC Router，手动 `/model` 切换易遗漏，先通过子任务分流省钱
- **待办**：装好 CC Router 后切换到 Flash 默认
...
发现并修复：
1. **API Key 泄露**：CLAUDE.md 末尾硬编码了 QWEN_API_KEY 完整密钥，已删除。需轮换该密钥。
2. **CC Router 未配置**：当前用户 Joe 下不存在 `.claude-code-router/config.json`，Flash 默认策略未生效。
3. **session-start.ps1 路径断裂**：`$transcriptDir` 指向旧项目 `g--Claude`，30/50轮预警实际不工作。已修复为三目录聚合查找。
4. **模型路由规则化**：新增 CLAUDE.md 规则 21-25，三层路由（本地→Flash→v4pro）+ 任务分类速查表。
5. **backup-laptop 配置过时**：全是旧用户 YU 的路径，对新环境无意义。

## 优化要点
- ⚠️ **最重要**：长会话分段（50 轮红线），单独此项可比今天省 ¥25+
- **模型路由（新）**：本地优先 → Flash 默认 → v4pro 显式切换（规则 21-25）
- 简单任务（文件读写、搜索、bash）默认走 flash
- 复杂任务（多文件重构、架构设计）手动切 v4-pro
- 长对话及时总结压缩上下文
- 确定性操作（格式转换、数据处理）用代码脚本而非 LLM
- **待办**：配置 CC Router 使 Flash 默认生效
